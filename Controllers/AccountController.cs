    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                // Use the new, more secure password verification method
                if (user != null && VerifyPassword(model.Password, user.Password))
                {
                    ...
                    // Redirect based on the corrected role
        ...
        // A more secure hashing method using HMACSHA512 with a hardcoded key.
        // NOTE: For production, store the key securely (e.g., in a secrets manager).
        private string HashPassword(string password)
        {
            var key = Encoding.UTF8.GetBytes("YourSecureSecretKey123"); // Replace with a secure key
        ...
        // Updated method to handle HR redirection
        private string GetControllerByRole(UserRole role)
        {
            return role switch
            {
                ...
                UserRole.HR => "HR", // Added new case for HR
                _ => "Home"
            };
        }
    }
